...
- Status: incomplete after Autopilot Package 5 implementation. The production watcher now progresses through durable triage/worktree preparation into one continuing, compacting, private Neon PR-owner instance per watch. Complete authoritative review/CI/readiness/authority/workspace envelopes, immutable grounding/receipt linkage, explicit monotonic memory/config cursors, trusted revision-bound read/fix capabilities, baseline-aware in-place re-grounding or audited capability rotation, serialized/byte-bounded coalesced turns, durable stop/supersession mutation epochs, under-lock live-head/identity/policy/effect checks, owner-specific restart reconciliation, bounded one-time submission persistence, and restart-safe deterministic prepared-diff submission are implemented. Package 5 now reserves and dispatches verification, admission-bound SHA/policy approval, non-force push, pushed-SHA top-level/thread result delivery, and terminal archival/grace-period cleanup through the same coordinator. Its focused fixtures pass, while the mandated full mode-matrix/bare-remote integration exit gate remains open. Explicit setup/control UX and full product-path integration/smoke remain open in Packages 6–8. The source-of-truth closure plan is `.plans/AUTOPILOT_IMPLEMENTATION_PLAN.md`; the evidence audit is `.plans/AUTOPILOT_END_TO_END_REVIEW.html`.
...
- [x] Connect `push_pr_autofix` to the durable coordinator with admission-bound approval, noninteractive credential, current-HEAD/policy revalidation, and canonical pushed-SHA persistence.
...
- [x] Connect `comment_pr_autofix_result` through the durable coordinator so it reserves admission-scoped idempotent top-level and review-thread deliveries for the pushed SHA.
