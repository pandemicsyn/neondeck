...
the planning-index policy only once complete. The operator decision above supersedes the earlier merge hold; deferred tests remain tracked.
...
Deferred by the operator decision above, not passed: live human iteration, version comparison and exact-version release;
...
acceptance. Retain this plan to track those deferred checks. Coding execution belongs to a subsequent slice.
