...
## 2026-07-19 - Autopilot Package 6 Shared Setup And Bounded Controls

- Roadmap item: Phases 19–20 / Autopilot Package 6 explicit setup and control surfaces.
- Decision: Implement one setup service and typed Flue action for watch creation/update, stable per-watch mode override, awaiting-event owner binding, process-existing choice, and readiness summary. API, CLI, dashboard setup, and chat use that contract; pause/resume/stop/retry/list/status share one control service. Stop cancels known nonterminal admissions before removing the watch, but Package 7 remains responsible for canonical admission history, pagination, and complete recovery controls.
- Reason: Reusing Package 4’s existing owner row preserves its lazy Flue-instance/worktree allocation and avoids a schema migration. A watch-specific upsert is protected by a token-owned, stale-recoverable cross-process filesystem lease, so concurrent CLI/server/chat writers preserve sibling overrides. Because the watch, JSON config, and owner row span different durable stores, setup records a strictly validated, lock-serialized durable fail-closed recovery marker before the first mutation; polling/admission ignores that watch until all boundaries complete or the same setup is retried. Stop revokes the active owner binding without deleting Package 4 audit history, so a later ordinary re-watch of the same PR cannot inherit Autopilot authority. Raw override-array mutation is not a display-assistant capability. Authority comparison asks for confirmation only when that watch gains capability.
- Follow-up: Package 5 owns verification/delivery progression. Package 7 must replace the current derived operator state with canonical history and expose complete lifecycle recovery. Package 8 retains full product-path, live-provider, and restart verification.
