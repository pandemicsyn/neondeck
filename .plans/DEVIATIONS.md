...
- Decision: Meaningful typed repository add/update/remove and policy changes
  ...
  observed stale asynchronous discovery results must retry. This serialization
  applies within the owning process only; another process can race the check/write
  window. File replacement failure leaves conservative revocation in place.
  No multi-process compare-and-set or cross-store transaction is claimed.
