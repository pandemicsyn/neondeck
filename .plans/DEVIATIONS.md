...
- Reason: Reusing Package 4’s existing owner row preserves its lazy Flue-instance/worktree allocation and avoids a schema migration. A watch-specific upsert prevents changing one PR from replacing sibling overrides, and authority comparison asks for confirmation only when that watch gains capability.
