...
## 2026-08-16 - Scheduled Task Workspaces V1 Boundaries

- Roadmap item: Scheduled repository workspace extension / `.plans/SCHEDULED_TASK_WORKSPACES_PLAN.md`.
- Decision: Implemented provider-neutral scheduled workspaces, local managed worktrees, application-owned exe.dev lifecycle, generic SSH hosts, exact Git preparation, a private dispatch-only Flue worker with run-stable uniquely named Flue skills, persisted artifacts, restart recovery, and the Scheduled Tasks dashboard. `delivery-enabled` is reserved for a future separately bound delivery tool and currently has the same effective shell capabilities as `trusted-workspace`. Remote shell workspaces require a dedicated credential-free SSH account, use a private HOME plus credential-disabled Git and SSH environment, derive clone URLs from credential-free GitHub identity instead of copying local remotes, and close disposable SSH connections after each sandbox operation. Run/tool authorization is rechecked against the active run and lock owner, with heartbeat renewal for both scheduled and linked local-worktree leases. Atomic cleanup leases exclude acquisition; existing resources detach without infrastructure deletion. Remote provisioning records durable intent before side effects, local orphan worktree intents are reconciled, and run-keyed immutable workspace snapshots keep later reuse from rewriting old occurrence detail. Local and canonical provider-resource locks serialize shared workspaces across tasks and with Autopilot while distinct generic-SSH roots on one host may run in parallel. Local direct branches are supported only when the branch is not held by another checkout; selecting `main` while the primary checkout holds it fails without mutating the primary checkout. Pinned mode requires a full SHA and exact direct-branch match; persistent task branches are continuation-only. Provider setup is configured through typed runtime config plus lifecycle-aware readiness; incompatible provider removal/replacement is blocked while durable resources depend on that adapter, remote roots are normalized absolute non-root POSIX paths, and the dashboard defaults repository tasks to local and reports readiness but does not write environment-variable names into config. Phase timing columns and an automatic delivered-success cleanup grace timer were not added; lifecycle timestamps, command durations, and cleanup attempt state are persisted instead.
- Reason: Bound push tools need the same exact remote-head, actor, destination, non-force, and reviewed-revision guarantees as Autopilot delivery and should not be approximated by exposing ambient SSH/Git credentials. Environment values must still be installed outside the browser, so a partial provider form would imply setup it cannot finish. The current state machine has enough durable boundaries for restart-safe admission, settlement, retention, and explicit cleanup without introducing another timer/coordinator solely for grace-period deletion.
- Security note: Local repository schedules deliberately use Flue's trusted `local()` adapter. Neondeck constrains structured file operations and the default command working directory to the managed worktree, but an authorized shell is not an OS, filesystem, network, or generic-delivery boundary and can address host paths or external services from inside a command. The implementation therefore does not use shell-token parsing as an authority control. Use exe.dev or a dedicated credential-free SSH host when host isolation is required; credential stripping and remote account separation reduce ambient authority but do not make arbitrary shell network-safe.
- Hardening detail: Recurring shell/network schedules, every provider-config mutation, and operator-triggered cleanup or detach require explicit confirmation. Provider and execution-policy changes share the durable admission coordinator; each workspace persists a required immutable validated adapter snapshot and physical identity, and provider-independent SSH user/host/port/canonical-root keys serialize aliases across provider ids. Settlement uses a renewable cross-process collection lease so only its owner records redacted, UTF-8-byte-bounded artifacts and terminalizes the workspace in the same database transaction as both the scheduled physical lock and any shared local-worktree/Autopilot lock release. Lost run authority aborts active command signals while Flue-compatible `exists` calls return false. A post-command audit persistence failure is owner-fenced, recorded separately, and never changes the completed `ShellResult`. Local credential-isolated HOME directories live outside worktrees and are cleaned with their app-owned lifecycle; generic SSH private HOME directories and existing workspace roots are canonicalized at attach time and removed only through those immutable paths on cleanup or detach. Generic SSH cleanup treats an already-missing provider tree as converged success, while provider-specific infrastructure deletion still reports adapter failures factually. The built-in exe.dev compatibility provider cannot be removed, and provider removal is blocked while execution policy references it. Pinned revisions fetch the declared ref when needed and verify the exact full commit.
- Final hardening: New scheduled local workspaces use a schedule-owned no-reuse worktree creation contract; `existing` accepts only explicitly adopted, ready, workflow-unowned worktree records and never the repository root. Worktree lock acquisition and scheduled-workspace binding commit together, while provider-backed approved execution participates in the same canonical physical-resource and linked-worktree locking. Explicit SSH key references are required and fail closed when absent or unreadable; host values reject URI/user/path/whitespace forms and canonical IPv6 identities are encoded unambiguously. Skip overlap decisions now create terminal auditable run rows while `queue-one` preserves its due occurrence. The configured default remote is exposed by the provider API and offered in the editor without replacing local as the overall task default. Provider connection-close failures after successful preparation or collection are redacted audit artifacts and cannot replace completed effects.
- Final review hardening: Cleanup, detach, lease renewal, and cleanup finalization are fenced by the immutable expected run id so historical run actions cannot affect a reusable workspace rebound to a later occurrence. Approved remote execution branches on the persisted adapter driver and uses its configured checkout root; root-derived attachment identities keep private HOME directories distinct. Generated per-run schedule branches are removed only after their schedule-owned worktree is gone and cleanup retries converge if branch removal was interrupted. SSH hosts normalize equivalent numeric IP spellings, while SSH command cancellation is truthfully reported as orphan-possible. Execution and provider failures are redacted before UTF-8 byte bounds at durable/API boundaries, Git evidence streams through a bounded FIFO with cleanup traps, editor state normalizes the Git/revision/lifecycle/overlap matrix, and provider config references are validated against both execution policy and saved task specs. Preparation connection-close artifacts use an exact claimed-or-active run-owner fence.
- Ninth review hardening: Remote SSH abort, timeout, output-limit, and post-mutation authority-loss outcomes create a durable app-state quarantine before an owner releases its physical lock. The quarantine survives restart, blocks all ordinary acquisitions after leases expire, and can be removed only by confirmed managed cleanup or an explicit operator attestation exposed through the API, Flue action, and Scheduled Tasks dashboard. Targetless approved remote commands now select the canonical configured provider root while using a separate root-derived private-HOME identity; approval scope persists the exact adapter fingerprint, canonical resource root, physical identity, checkout target, and forwarded-environment audit, and execution refuses drift. Provider replacement is blocked while pending or reusable approvals reference it. Task creation and repository/provider removal share the admission coordinator and validate saved task plus material-resource references. Selected runtime skills are rejected before dispatch when their Flue names collide with workspace-discovered skills. Disposable SSH transport-close failures are redacted, recorded separately, and never reject an already settled sandbox operation.
- Tenth review hardening: Explicit quarantine clearance now removes the exact expired physical lease and its quarantine atomically, while refusing an active or replaced owner. SSH structured writes are byte-bounded and all structured mutations have an infrastructure deadline; timed-out or transport-failed mutations and post-launch command stream failures are typed as orphan-possible outcomes and feed the same durable quarantine path. If quarantine persistence itself fails, execution is fenced first and the current physical lease is preserved so expiry recovery fails closed. Scheduled admission and config writes renew and revalidate their shared coordinator lease. Each new workspace and immutable run snapshot records the exact repository id, GitHub owner/name, local path, and default branch admitted under that coordinator; legacy rows are explicitly unverified and cleanup refuses to use their repository path for branch deletion. Repository identity retargeting requires confirmation and is blocked by saved tasks or material workspaces. Manual Run now/Retry uses a durable one-occurrence marker, so a paused recurring schedule stays paused. Scheduled run messages, errors, and result JSON are redacted and UTF-8-byte-bounded at persistence and read boundaries.
- Eleventh review hardening: Remote workspaces persist an owner-fenced physical-release marker before recording an uncertain operation; terminal failure preserves the physical lease when that marker is present, restart recovery conservatively quarantines inherited SSH ownership, and explicit quarantine clearance atomically removes the expired lease, quarantine, and release fence. Neondeck uses a conforming hand-written Flue `Sandbox` wrapper for SSH so only factual missing-parent writes retry; all structured reads, metadata calls, recursive directory creation, and mutations are byte-bounded or deadlined. Historical run-snapshot migrations backfill an explicitly unverified repository identity and the release-fence field. Legacy managed-worktree cleanup now converges after safe worktree removal while retaining an unverifiable generated branch as an audited orphan instead of looping forever. Repository config writes recheck the shared coordinator owner immediately before every file/database side effect. Scheduled result persistence rebuilds, redacts, and UTF-8-byte-bounds the final serialized JSON after a virtual response is merged; task deletion removes its pending manual-occurrence marker transactionally. The dashboard surfaces the immutable repository and provider-resource identities in task/run detail.
- Twelfth review hardening: Every physical-resource lease now persists its exact provider id. The shared release primitive preserves exact leases whenever any durable workspace fence or quarantine exists, so failure, retention, expired collection recovery, and cleanup finalization cannot bypass the no-release state. Fence-only state remains visible and clearable after a quarantine-write failure; clearance refuses live ownership and atomically removes only an expired exact lease, quarantine, and inactive fence. Disposable remote `Sandbox.exec` rejects an already-aborted command before connecting, races cancellation and the command deadline through SSH/security setup, and closes late connections without creating the isolated HOME or launching the requested command. Production SFTP preserves only coded missing-parent failures for the conforming one-time parent-create retry; typed uncertainty is never masked. Writable scheduled sandboxes compose Flue 2.0.3's standard write and edit tools in addition to read/search/shell, and the new adapter code uses the current `Sandbox` and `SandboxDriver` names rather than deprecated aliases.
- Thirteenth review hardening: Managed provider cleanup now has a distinct owner-fenced success transition that verifies the exact live cleanup owner and lease, then atomically terminalizes the workspace and removes its physical lease, release fence, and quarantine; stale owners are refused, while ordinary operator clearance remains expiry-gated. Disposable remote sandbox setup and every filesystem or command verb report typed uncertainty through one application callback before propagation. Scheduled execution persists the release fence and quarantine, invalidates the exact run, and makes `exists` return `false` only after that fail-closed handling, including private-HOME setup failures on read-only operations.
- Follow-up: Add scheduled delivery only by composing the existing revision-bound delivery/readiness gates. Add an environment-reference provider setup editor when it can validate and persist the complete non-secret configuration. Add optional grace-period cleanup as a reconciliation job if retained delivered-success workspaces become operationally noisy.
