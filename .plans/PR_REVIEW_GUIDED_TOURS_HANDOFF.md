...
Companion to `PR_REVIEW_GUIDED_TOURS_PLAN.md`. The plan says *what* and *why*.
This says *where in this codebase*, *with which values*, and *what will bite
you*. Every file:line reference below was read at the time of writing; treat
...
If you *do* want to edit a board, rebuild instructions and the two silent traps
...
```ts
const traversalKinds = ['file','hunk','review-thread','local-draft','finding','attention']
```
...
See the plan's *Findings As Tour Sources* and the `ShowWhy` board. The short
...
See the plan's *Walk and read* and the `Reading` board. The argument in one
...
.pr-review-tour-location { color: var(--tour); font-weight: 600; }
.pr-review-tour-symbol   { color: var(--muted); }
...
.pr-review-tour-copy { margin: 0; max-width: 76ch; }
```
...
.pr-review-tour-marker:hover { border-color: var(--tour); color: var(--ink); }
```
...
```css
.pr-review-tour-spine li { position: relative; display: flex; gap: 8px; padding-bottom: 10px; }
...
.pr-review-tour-spine li:last-child::before { display: none; }
```

Step states are `done | now | todo`. **`done` and `todo` render the same**
(`--muted`); only `now` takes `--tour` and `600`. If you add a distinct `done`
treatment, make sure it is not *brighter* than `todo` — that bug was caught in
...
styled by *both* the shadow `unsafeCSS` in `theme.ts` (for inherited properties
— font, colour, custom properties) *and* the app stylesheet (for everything
...
first. The plan's sequence is a dependency order for the *complete* feature;
