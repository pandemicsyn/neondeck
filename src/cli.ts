...
mcp
  ...
  .action(async (id: string, options: { confirm?: boolean }) => {
    const { removeMcpServer, getMcpRegistry } = await mcpModule();
    const paths = await pathsFromOptions(program.opts<GlobalOptions>());
    loadEnvForPaths(paths);
...
mcp
  ...
  .action(async (id: string) => {
    const { setMcpServerEnabled, getMcpRegistry } = await mcpModule();
    const paths = await pathsFromOptions(program.opts<GlobalOptions>());
    loadEnvForPaths(paths);
...
mcp
  ...
  .action(async (options: McpApprovalOptions) => {
    const { listMcpApprovals, resolveMcpApprovalWithPaths } = await mcpModule();
    const paths = await pathsFromOptions(program.opts<GlobalOptions>());
    loadEnvForPaths(paths);
    if (options.resolve) {
      const decision = options.approve
        ...
        : options.deny
          ? 'deny'
          : null;
      if (!decision) throw new Error('--resolve requires --approve or --deny');
      printActionResult(
