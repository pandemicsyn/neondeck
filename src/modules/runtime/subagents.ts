...
export type ExploreSubagentSelection = {
  model: string;
  thinkingLevel: AgentModelSelection['subagentThinkingLevels']['explore'];
};

export function resolveExploreSubagentSelection(
  captured: Partial<ExploreSubagentSelection>,
  fallback: ExploreSubagentSelection,
): ExploreSubagentSelection {
  return {
    model: captured.model ?? fallback.model,
    thinkingLevel: captured.thinkingLevel ?? fallback.thinkingLevel,
  };
}
