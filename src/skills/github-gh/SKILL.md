...
- Watched-PR delivery uses only the continuing owner's mode-scoped push tool. Autonomous safe mode rechecks the current mode, exact non-force destination, configured checks, and current worktree state immediately before push; approval mode receives push authority only on the direct human turn while the commit is held for review.
