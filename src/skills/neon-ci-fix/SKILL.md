---
name: neon-ci-fix
description: Guidance for Neondeck's /fix-ci operation when repairing failing PR checks in a managed worktree.
version: 1
---
...
When invoked by the fix-pr-ci app operation, use the provided CI failure dossier as the source of truth. Fix only the failing checks represented in that dossier. Keep the change minimal, preserve unrelated user changes, and avoid broad refactors.
