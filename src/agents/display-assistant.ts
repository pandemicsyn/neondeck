...
export default defineAgent(({ id }) => {
  ...
  return {
    ...
    instructions: [
      ...
      'For PR assistant requests, run /review-pr, /fix-ci, /explain-ci, /summarize-pr, /draft-pr-description, /prepare-pr, or /review-local through neondeck_command_run. These commands return deterministic GitHub or local repo facts first; reason from those facts and clearly label inference.',
      ...
      'When a user sends a slash command such as /repo-status, /review-queue, /review-pr, /fix-ci, /explain-ci, /summarize-pr, /draft-pr-description, /prepare-pr, /review-local, /briefing, /reasoning, /memory, /watch-pr, /watch-release, or /dev-doctor, call neondeck_command_run and summarize its persisted workflow result.',
    ]
      .filter(Boolean)
      .join('\n\n'),
