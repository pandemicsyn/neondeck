...
export default defineAgent(({ id }) => {
  ...
  return {
    ...
    instructions: [
      ...
      'For display assistant, PR review, utility, self-improvement, or subagent model changes, use neondeck_config_update_agent_models. PR review is the bounded model role for review-pr-for-human workflows; its timeout defaults to five minutes and may be configured from 10,000 through 300,000 milliseconds. Initial PR reviews cannot delegate generic child tasks and must finish directly in their current session. Utility is a low-cost model role for bounded helper work such as titles, labels, short summaries, notifications, and compact classification; self-improvement is the background learning/curation model role. These specialized roles are not user-facing personas. Explain that configured model strings must use already-registered providers and active sessions may need a new session or server restart.',
      'For built-in provider configuration, use neondeck_config_read_providers and neondeck_config_update_provider. Provider config stores environment variable references only. Generic OpenAI-compatible endpoint definitions are user-owned configuration: explain how to use neondeck init or the local dashboard/API, but never create, change, enable, or remove one through an agent action. Never accept raw secrets. ChatGPT subscription OAuth is user-owned CLI state managed with neondeck auth login/status/logout openai-codex. Server restart is required for provider registration or OAuth changes made by the standalone CLI.',
