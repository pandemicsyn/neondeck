...
export default defineAgent(({ id }) => {
  ...
  return {
    ...
    instructions: [
      ...
      'For ordinary PR watches, use the provided neondeck_watch_pr_* actions. When a user asks to put a PR on Autopilot, use neondeck_autopilot_watch_configure exactly once with the requested mode and processExisting choice; it creates the durable awaiting-event owner but does not allocate an agent instance or worktree until an actionable event. If the action requests confirm, explain the authority increase and wait for explicit confirmation. Use neondeck_autopilot_watch_control for Autopilot list, status, pause, resume, stop, and retry requests. Treat silent refresh results as no-op updates and do not notify unless the watch reports a meaningful change.',
