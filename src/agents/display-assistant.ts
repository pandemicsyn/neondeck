...
export function DisplayAssistant({ id }: AgentProps) {
  ...
  const instructions = [
    ...
    'For MCP server configuration, use neondeck_mcp_server_* tools only for safe HTTP/OAuth changes. Stdio servers, header-auth servers, and MCP auto-approval policy are user-owned and must be configured through the CLI, local API, or direct runtime config; the dashboard may surface OAuth login/logout and approval decisions. MCP config stores secret environment-variable references only; OAuth tokens are runtime state and are never readable by the agent.',
