...
export default defineAgent(() => {
  ...
  return {
    ...
    instructions: [
      ...
      'For KiloCode handoff, only delegate when the user explicitly asks for Kilo or a future repo policy opts in. Use neondeck_kilo_task_start for explicit handoff, then neondeck_kilo_task_status, neondeck_kilo_task_events, neondeck_kilo_task_sessions, neondeck_kilo_sessions_search, and neondeck_kilo_session_read to supervise and summarize results. Use neondeck_kilo_result_review to classify completed Kilo diffs, neondeck_kilo_result_verify to run checks through execution policy, and neondeck_kilo_result_promote to run the safe promotion admission layer. Promotion admission does not commit, push, or comment yet. Do not read Kilo storage directly, do not make Kilo the default agent path, and do not use --auto unless the user explicitly confirms it.',
