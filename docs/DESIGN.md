...
The structure underneath is disciplined: a sticky blurred nav, a centered hero with a copyable install command, a hairline-gridded feature wall, a config split with a code window, and a docs shell that shares the exact same chrome. There are two public themes: `dark` (implemented as the default `camo` look, with full warped stripes) and `light` (cool paper for daylight), driven by a `data-look` / `data-theme` attribute pair persisted to localStorage.
...
- Two public themes: dark/camo and light.
...
- **Hero primary (signature):** pink fill with a `0 0 28px` pink glow; hover flips to cyan.
...
- **Do** keep the hero camo decorative and behind the vignette so headline text always meets AA across dark/camo and light.
