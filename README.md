...
Handled PR/autopilot outcomes are also counted for learning with idempotent `pr_handled` source ids. When the handled-event count reaches `learning.prRetrospectiveThreshold`, or when `POST /api/learning/reviews/prs` is called manually, `review_pr_batch_for_learning` can propose project/local memory and skill patch candidates. Inspect candidates with `GET /api/learning/candidates` or `GET /api/skills/patches`, then apply or reject them through the matching local API routes.
