---
'neondeck': minor
---

Give PR reviews exact-revision read-only repository traversal, bounded per-file anchor validation, configurable full prompts for initial and follow-up reviewers, a revision-scoped durable reviewer conversation with stable per-question context loading, revision-correlated live GitHub review-thread awareness, connection recovery, and clearer inline GitHub threads.

Keep packaged React runtime dependencies on one exact patch version so fresh installs boot reliably.
